...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    private static final String TAG = "Joy.PagedView";
    ...
    private static final float SIGNIFICANT_MOVE_THRESHOLD = 0.2f;//modify by yongjian.he from 0.4f to 0.2f.
    ...
    protected void init() {
        ...
        //modify by xiong.chen for wxy-432 at 2013-07-08
        isCycle = PreferencesProvider.Interface.General.getCycleScrollMode();
        //add end
    ...
    protected float getScrollProgress(int screenScroll, View v, int page) {
		...
		} else if ((mOverScrollX > mMaxScrollX || mOverScrollY > mMaxScrollY) && page == 0 && isCycle) {
    ...
    protected void snapToPage(int whichPage, int delta, int duration) {
        ...
        if (!mVertical) {
            ...
            int childCount = getChildCount();
            int offset = childCount * height;
            int loopDelta = 2 * height;
            if (childCount <= 2 && childCount != 0) {
            	loopDelta = height;
    		}
            if (delta > loopDelta) {
    			delta -= offset;
    			mUnboundedScrollY += offset;
    		}
            if (delta < -loopDelta) {
    			delta += offset;
    			mUnboundedScrollY -= offset;
